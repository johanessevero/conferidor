...
/**
 * 
 * @author Johanes Severo dos Santos Classe que modela a apresentacao da
 *         interface para a conferencia da planilha.
 */
public class ApresentacaoConferencia extends JFrame implements ActionListener {
	...
	public ApresentacaoConferencia() throws DAOException {
		...
		jmEnviarRelatorioErros = new JMenuItem(
				"  Enviar relatorio de erros para o email do setor  ");
		...
		btElaborarEscala = new JButton("Elaborar escala prox mes");
		btElaborarEscala.addActionListener(this);
		btElaborarEscala.setEnabled(false);
		...
		spNomesRelatorio = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true,
				new JScrollPane(pRelatoriosServidores), new JScrollPane(
						pTextoRelatorio));
	...
	private void mostraBotoesNomes(boolean arquivo)
			throws NumberFormatException, RNException {

		if (arquivo) {

			if (cbFormato.getSelectedIndex() == 0)
				servidores = leitor.getDadosPlanilhaEscalaXML(
						nomePlanilhaLeitura, cbMeses.getSelectedIndex() + 1,
						Integer.parseInt((String) cbAno.getSelectedItem()));

			else
				servidores = leitor.getDadosPlanilhaEscala(nomePlanilhaLeitura,
						cbMeses.getSelectedIndex() + 1,
						Integer.parseInt((String) cbAno.getSelectedItem()));

			conferidor.conferirEscala(servidores, true);
		} else {
			conferidor.conferirEscala(servidores, true);
		}
		...
		pRelatoriosServidores.setLayout((new GridLayout(servidores.length + 1,
				1)));
		...
		for (int i = 0; i < servidores.length; ++i) {
			...
			relatorio = conferidor.getRelatorioErro(servidores[i]
					.getMatricula());
			...
			float falta = conferidor.calculaFaltaUltimaSemana(servidores[i]
					.getEscala().getHorasUltimaSemana(), servidores[i]
					.getCargaHoraria());

			if (!relatorio.isOk()) {

				btRelatorioErrosServidores[i].setText("ERRO: "
						+ servidores[i].getMatricula()
						+ " "
						+ servidores[i].getNome()
						+ " "
						+ AuxiliarConstantes.getDiaSemanaData(
								AuxiliarConstantes.getUltimoDiaMes(
										servidores[i].getEscala().getMes(),
										servidores[i].getEscala().getAno()),
								servidores[i].getEscala().getMes(),
								servidores[i].getEscala().getAno()) + " "
						+ falta + " THE: "
						+ servidores[i].getEscala().getTotalHorasExtras());
				btRelatorioErrosServidores[i].setForeground(Color.RED);
				btRelatorioErrosServidores[i].setFont(new Font("Monospaced",
						Font.ITALIC + Font.BOLD, 14));

			} else {

				btRelatorioErrosServidores[i].setText("OK: "
						+ servidores[i].getMatricula()
						+ " "
						+ servidores[i].getNome()
						+ " "
			...
			quantidadeHorasExtras += servidores[i].getEscala()
					.getTotalHorasExtras();

		}

		lbHorasExtras
				.setText("Total de horas extras: " + quantidadeHorasExtras);
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (e.getSource() == btElaborarEscala) {
				...
				if (servidores != null)
					...
					JOptionPane.showMessageDialog(null,
							"Nao ha uma conferencia feita!", "Erro",
							JOptionPane.ERROR_MESSAGE);

			} else if (e.getSource() == jmEnviarRelatorioErros) {

				if (servidores != null) {
					String nomeArq = geracaoArquivos.geraRelatorioErros(
							servidores, conferidor, servidores[0].getSetor());
					try {
						if (!lbEmail.getText().isEmpty()) {
							email.enviarEmailArquivo("Relatorio de erros",
									"Segue o relatorio de erros.", lbEmail
											.getText().trim(), nomeArq.trim());
							JOptionPane.showMessageDialog(null,
									"Relatorio enviado com sucesso para: "
											+ lbEmail.getText() + "!", "Erro",
									JOptionPane.INFORMATION_MESSAGE);

						} else
							JOptionPane.showMessageDialog(null,
									"Este setor não tem um email cadastrado!",
									"Erro", JOptionPane.ERROR_MESSAGE);

					} catch (MessagingException e1) {
						JOptionPane.showMessageDialog(null,
								"Erro ao enviar o email!", "Erro",
								JOptionPane.ERROR_MESSAGE);
					}
				} else
					JOptionPane.showMessageDialog(null,
			...
			} else if (e.getSource() == jmExtra) {

				if (servidores != null)
					geracaoArquivos.geraProcessoHorasExtras(servidores,
							cbMeses.getSelectedIndex() + 1,
							Integer.parseInt((String) cbAno.getSelectedItem()),
							leitor.getValoresHoraExtra());
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha uma conferencia feita!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == jmEscaladosNoDia) {

				if (servidores != null)
					new ApresentacaoEscolhaDia(geracaoArquivos,
							servidores[0].getSetor(), servidores);
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha uma conferencia feita!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == jmTesouraria) {
				if (servidores != null)
					geracaoArquivos.geraRelatorioQuantidadeRefeicoes(
							servidores, servidores[0].getSetor());
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha uma conferencia feita!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == jmQuantitativoPeriodo) {
				if (servidores != null)
					geracaoArquivos.geraQuantitativoPorPeriodo(servidores,
							servidores[0].getSetor(), servidores[0].getEscala()
									.getMes(), servidores[0].getEscala()
									.getAno());
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha uma conferencia feita!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == jmRascunho) {

				if (servidores != null)
					geracaoArquivos.geraRascunho(servidores, servidores[0]
							.getSetor(), servidores[0].getEscala().getMes(),
							servidores[0].getEscala().getAno());
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha conferencia feita!!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == jmErros) {

				if (servidores != null)
					geracaoArquivos.geraRelatorioErros(servidores, conferidor,
							servidores[0].getSetor());
				else
					JOptionPane.showMessageDialog(null,
							"Nao ha conferencia feita!!", "Erro",
							JOptionPane.ERROR_MESSAGE);
			} else if (e.getSource() == btConferir) {

				if (nomePlanilhaLeitura.equals(""))
					JOptionPane.showMessageDialog(null,
							"Escolha um arquivo para coferencia.", "Erro",
							JOptionPane.ERROR_MESSAGE);
				else {
					mostraBotoesNomes(true);
				}
			} else if (e.getSource() == btVisualizarEscala) {

				ApresentacaoEscalaServidores aes = new ApresentacaoEscalaServidores(
						servidores, true, cbMeses.getSelectedIndex() + 1,
						new File(nomePlanilhaLeitura).getPath());
			...
			} else if (e.getSource() == btConferirTudo) {

				File arq = new File(txArquivo.getText());
				if (arq.isDirectory()) {
					conferidor.conferirTudo(leitor, txArquivo.getText().trim(),
							cbFormato.getSelectedIndex(),
							cbMeses.getSelectedIndex() + 1,
							Integer.parseInt((String) cbAno.getSelectedItem()));
					JOptionPane.showMessageDialog(null,
							"Escalas conferidas com sucesso!", "Erro",
							JOptionPane.INFORMATION_MESSAGE);

				} else
					JOptionPane.showMessageDialog(null,
							"Escolha um diretorio para conferencia!", "Erro",
							JOptionPane.ERROR_MESSAGE);
				...
				if (servidores != null) {
					...
					for (int i = 0; i < servidores.length; ++i) {
						...
						if (e.getSource() == btRelatorioErrosServidores[i]) {
							...
							EntidadeMensagem entidadeMensagem = conferidor
									.getRelatorioErro(servidores[i]
											.getMatricula());

							// taRelatorio.setText(conferidor.getRelatorioErro(servidores[i].getMatricula()).getMensagem())
							int posChr = 0;
							while (posChr < entidadeMensagem.getMensagem()
									.length()) {
								String parteMensagem = "";
								aqui: for (int k = posChr; k < entidadeMensagem
										.getMensagem().length(); ++k) {

									if (entidadeMensagem.getMensagem()
											.charAt(k) != '$')
										parteMensagem += entidadeMensagem
												.getMensagem().charAt(k);

									if (entidadeMensagem.getMensagem()
											.charAt(k) == '$') {

										taRelatorio
												.append(parteMensagem + "\n");
