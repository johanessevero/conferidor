...
/**
 * 
 * @author Johanes Severo dos Santos Classe que implementa a interface de
 *         apresentacao da escala de um servidor.
 */
public class ApresentacaoEscalaServidor extends JPanel implements
		ActionListener {
	...
	private JLabel lbRotuloQuantidadeRefeicoes, lbQuantidadeRefeicoesDiurno,
			lbQuantidadeRefeicoesNoturno;
	private JLabel lbRotuloTotalHorasExtras, lbTotalHorasExtras;
	private JLabel lbRotuloCompensacao, lbCompensacao;

	private JTextField txMatricula, txNome, txCargo, txCargaHoraria,
			txCargaHorariaSetor, txPassagemMesAnterior;
	...
	public ApresentacaoEscalaServidor(EntidadeServidor servidor,
			boolean habilitaCamposEscala) {
	...
	private void montaLayout(EntidadeServidor servidor) {
		...
		try {
			...
			int ultimoDiaMes = AuxiliarConstantes.getUltimoDiaMes(servidor
					.getEscala().getMes(), servidor.getEscala().getAno());
			...
			txMatricula.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			txCargaHoraria = new JTextField(Integer.toString(servidor
					.getCargaHoraria()));
			txCargaHoraria.setForeground(Color.BLUE);
			txCargaHoraria.setFont(new Font("Monospaced", Font.ITALIC
					+ Font.BOLD, 14));
			lbRotuloCargaHorariaSetor = new JLabel(" CARGA HORARIA NO SETOR: ");
			txCargaHorariaSetor = new JTextField(Integer.toString(servidor
					.getCargaHoraria()));
			txCargaHorariaSetor.setForeground(Color.BLUE);
			txCargaHorariaSetor.setFont(new Font("Monospaced", Font.ITALIC
					+ Font.BOLD, 14));
			...
			lbRotuloPassagemMesAnterior = new JLabel(
					"  PASSAGEM DO MES ANTERIOR: ");
			txPassagemMesAnterior = new JTextField(Float.toString(servidor
					.getEscala().getHorasUltimaSemanaMesAnterior()));
			txPassagemMesAnterior.setForeground(Color.BLUE);
			txPassagemMesAnterior.setFont(new Font("Monospaced", Font.ITALIC
					+ Font.BOLD, 14));
			...
			if (!servidor.getEscala().isEscalaLancada()) {
				...
				lbErro.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
						25));
				...
				lbErro.setFont(new Font("Monospaced", Font.BOLD + Font.ITALIC,
						25));
			 ...
			 * 
			 * int selecao = 0; for (int i = 0; i < setores.length; ++i) {
			 * cbSetor.addItem(setores[i]); if
			 * (servidor.getSetor().equals(setores[i])) { selecao = i; } }
			...
			lbRotuloQuantidadeRefeicoes = new JLabel(
					" | QUANTIDADE DE REFEICOES D & N: ");
			lbQuantidadeRefeicoesDiurno = new JLabel(servidor.getEscala()
					.getQuantidadeRefeicoesDiurno() + "");
			lbQuantidadeRefeicoesDiurno.setForeground(Color.BLUE);
			lbQuantidadeRefeicoesDiurno.setFont(new Font("Monospaced",
					Font.ITALIC + Font.BOLD, 14));
			lbQuantidadeRefeicoesNoturno = new JLabel(servidor.getEscala()
					.getQuantidadeRefeicoesNoturno() + "");
			lbQuantidadeRefeicoesNoturno.setForeground(Color.BLUE);
			lbQuantidadeRefeicoesNoturno.setFont(new Font("Monospaced",
					Font.ITALIC + Font.BOLD, 14));
			lbRotuloTotalHorasExtras = new JLabel(" | TOTAL DE HORAS EXTRAS: ");
			lbTotalHorasExtras = new JLabel(servidor.getEscala()
					.getTotalHorasExtras() + "");
			lbTotalHorasExtras.setForeground(Color.BLUE);
			lbTotalHorasExtras.setFont(new Font("Monospaced", Font.ITALIC
					+ Font.BOLD, 14));
			lbRotuloCompensacao = new JLabel(" | BANCO DE HORAS: ");
			lbCompensacao = new JLabel(servidor.getEscala()
					.getBancoHorasDestaEscala() + "");
			lbCompensacao.setForeground(Color.BLUE);
			lbCompensacao.setFont(new Font("Monospaced", Font.ITALIC
					+ Font.BOLD, 14));
			lbRotuloPassagemProximoMes = new JLabel(
					" | PASSAGEM PARA O PROXIMO MES: ");
			lbPassagemProximoMes = new JLabel(servidor.getEscala()
					.getHorasUltimaSemana() + "");
			...
			lbSemana1.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			lbSemana2.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			lbSemana3.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			lbSemana4.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			lbSemana5.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			lbSemana6.setFont(new Font("Monospaced", Font.ITALIC + Font.BOLD,
					14));
			...
			for (int i = 0; i < lbDiasMes.length; ++i) {
				...
				lbDiasSemana[i] = new JLabel(
						AuxiliarConstantes.getDiaSemanaData(i + 1, servidor
								.getEscala().getMes(), servidor.getEscala()
								.getAno()));
				txEscalaContratual[i] = new JTextField();

				if (servidor.getEscala() != null)
					txEscalaContratual[i].setText(servidor.getEscala()
							.getEscalaHoraContratual()[i]);

				if (i <= dataAtual[0] - 1
						&& servidor.getEscala().getMes() == dataAtual[1]
						&& servidor.getEscala().getAno() == dataAtual[2]
						&& habilitaCamposEscala == false) {
				...
				if (servidor.getEscala() != null)
					txEscalaExtra[i].setText(servidor.getEscala()
							.getEscalaHoraExtra()[i]);
			...
			for (int i = 0; i < lbDiasMes.length; ++i) {
				...
				if (lbDiasSemana[i].getText().equals("Dom")) {
					lbDiasSemana[i]
							.setText(" ||| " + lbDiasSemana[i].getText());
			...
			pDadosEscala.setBorder(BorderFactory.createLineBorder(Color.black,
					1));
			...
			pFuncionalidades.setBorder(BorderFactory.createLineBorder(
					Color.black, 1));
	...
	public EntidadeServidor getDados() {
		...
		servidor.setCargaHorariaSetor(Integer.parseInt(txCargaHorariaSetor
				.getText()));
		...
		escala.setHorasUltimaSemanaMesAnterior(Float
				.parseFloat(txPassagemMesAnterior.getText()));
		escala.setHorasUltimaSemana(Float.parseFloat(lbPassagemProximoMes
				.getText()));
		escala.setBancoHorasDestaEscala(Float.parseFloat(lbCompensacao
				.getText()));
		...
		escala.setTotalHorasExtras(Integer.parseInt(lbTotalHorasExtras
				.getText()));
		escala.setQuantidadeRefeicoesDiurno(Integer
				.parseInt(lbQuantidadeRefeicoesDiurno.getText()));
		escala.setQuantidadeRefeicoesDiurno(Integer
				.parseInt(lbQuantidadeRefeicoesNoturno.getText()));
		escala.setObservacao(txObservacao.getText());

		int ultimoDiaMes = AuxiliarConstantes.getUltimoDiaMes(escala.getMes(),
				escala.getAno());
	...
	public EntidadeMensagem validar(float passagemHoras) {
		...
		String escalaContratual[] = new String[AuxiliarConstantes
				.getUltimoDiaMes(servidor.getEscala().getMes(), servidor
						.getEscala().getAno())];
		String escalaExtra[] = new String[AuxiliarConstantes.getUltimoDiaMes(
				servidor.getEscala().getMes(), servidor.getEscala().getAno())];

		for (int i = 0; i < AuxiliarConstantes.getUltimoDiaMes(servidor
				.getEscala().getMes(), servidor.getEscala().getAno()); ++i) {

			escalaContratual[i] = AuxiliarGeral
					.substituiEspacos(txEscalaContratual[i].getText()).trim()
					.toUpperCase();
			escalaExtra[i] = AuxiliarGeral
					.substituiEspacos(txEscalaExtra[i].getText()).trim()
					.toUpperCase();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btValidar) {
			...
			String mensagem = validar(
					servidor.getEscala().getHorasUltimaSemana()).getMensagem()
					.replace('$', '\n');
			JOptionPane.showMessageDialog(null, mensagem, "Relatorio",
